...
# ============================================================================
"""AdamWeightDecayX, a customized Adam. Input: gradient, overflow flag."""
...
class AdamWeightDecayX(Optimizer):
   ...
   """
    def __init__(self, params, learning_rate=1e-3, beta1=0.9, beta2=0.999, eps=1e-6, weight_decay=0.0):
        super(AdamWeightDecayX, self).__init__(learning_rate, params, weight_decay)
        ...
        if isinstance(self.weight_decay, Sequence):
            self.weight_decay = self.weight_decay_tensor_tuple
        else:
            self.weight_decay = Tensor(self.weight_decay)
    ...
    def construct(self, gradients, overflow):
        ...
        beta1 = self.op_select(cond, self.op_cast(F.tuple_to_array((1.0,)), mstype.float32), self.op_cast(self.beta1, mstype.float32))
        beta2 = self.op_select(cond, self.op_cast(F.tuple_to_array((1.0,)), mstype.float32), self.op_cast(self.beta2, mstype.float32))
        if self.is_group:
            if self.is_group_lr:
